...
### 8.4 Redis 连接先不要改现有 6379

因为本任务先在 6380 做隔离实验，不能直接把当前 `spring.data.redis.port` 全局改成 6380，否则排行榜、面试上下文和限流也会切换到新实例。

第一轮推荐使用下一节的“手工测试配置”，只让测试中的 VectorStore 连接 6380。等最小闭环通过后，再在下一任务中决定：

1. 本地开发全部迁移到 Redis Stack；或
2. 缓存 Redis 与向量 Redis 使用两个独立连接。

第一种简单，第二种隔离性更好。当前阶段不急着做最终架构决定。
...
- `docs/RAG入门-阶段三前置知识.md`
- `docs/RAG在智面项目中的定位、实现与新手学习路线.md`
